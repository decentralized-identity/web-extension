...
(function(){
  ...
  function fireEvent(node, type, options = {}){
  ...
  customElements.define('modal-overlay', class SidePanels extends HTMLElement {
    static get observedAttributes() {
      return ['open'];
    }
    constructor() {
      ...
      this.addEventListener('transitionend', e => {
        if (e.target === this && e.propertyName === 'opacity') {
          fireEvent(this, this.isOpen ? 'modalopened' : 'modalclosed')
        }
      });
    }
    get isOpen (){
    ...
    attributeChangedCallback(attr, last, current) {
      switch(attr) {
        case 'open':
          domReady.then(e => {
            fireEvent(this, current !== null ? 'modalopen' : 'modalclose')
          })
        }
      }
  });
              ...
              if (node.tagName === 'SECTION') {
                if (current !== null) {   
                  if (node.offsetHeight < node.scrollHeight) {
                    ...
                    fireEvent(this, 'detailboxtoggle', { detail: { open: true } });
                  }
                }
                  ...
                  node.style.height = 0;
                  fireEvent(this, 'detailboxtoggle', { detail: { open: false } });
                }
                break;
              }
            }
